...
class esb (
  ...
  $target             = '/mnt',
  $monitoring         = false,
  $catalina_server_http_proxyport = '8280',
  $catalina_server_https_proxyport = '8243',
  $ipaddress_localMemberPort = '127.0.0.1',
  $port_mapping       = { 8280 => 9764, 8243 => 9444},
) inherits params {
notice("inheritis param sub_cluster_domain ${sub_cluster_domain} members ${members} port mapping ${port_mapping} version  ${version} ")

  $deployment_code = 'esb'
  $carbon_version  = $version
  $service_code    = 'esb'
  $carbon_home     = "${target}/wso2${service_code}-${carbon_version}"
notice("carbon_home : ${carbon_home}")
...
notice("$service_templates ${service_templates}")
  $common_templates = []
notice("service_code ${service_code}")
  tag($service_code)
notice("Starting clean")
